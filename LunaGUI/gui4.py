...
def the_reader_thread():
    ...
    while(True):
        ...
        if cmd=="INVTHW":
            ...
            list_box.insert(END, "Moved Left")
        elif cmd=="MOVERIGHT":
            list_box.insert(END, "Moved Right")
...
"""
DAVES LASER MOTRO MOTOR
"""

def move_laser_motor_left_button_click():
    """
    Button click to read data about the currently running sequence.
    :return:
    """
...
def move_laser_motor_right_button_click():
    """
    Button click to read data about the currently running sequence.
    :return:
    """
...
def capillary_maintenance_button_click():
    maintance_window = LunaUI()
    maintance_window.wm_title("Maintenance/Capillary")

    current_status_label = Label(maintance_window, text="Current Status")
    current_status_label.grid(row=0, column=0, columnspan=3)
    list_box = Listbox(maintance_window, width=40)
    list_box.grid(row=1, column=0, columnspan=3)

    # Buttons
    replace_button = Button(maintance_window, text="Replace Capillary", command=None)
    replace_button.grid(row=1, column=4)

    continue_button = Button(maintance_window, text="Continue", command=None)
    continue_button.grid(row=2, column=4)

    cancel_button = Button(maintance_window, text="Cancel", command=maintance_window.quit)
    cancel_button.grid(row=3, column=4)

    left_button = Button(maintance_window, text="Left", command=move_laser_motor_left_button_click)
    left_button.grid(row=3, column=0)

    right_button = Button(maintance_window, text="Right", command=move_laser_motor_right_button_click)
    right_button.grid(row=3, column=1)
    maintance_window.mainloop()
...
if __name__ == '__main__':
    ...
    main_window = LunaUI()
    main_window.wm_title("Main")

    # title_label = Label(luna, text="OPTOKEY", fg="red", font=("Helvetica", 16))
    enter_blood_sample_label = Label(main_window, text="Enter Blood Sample!")
    enter_blood_sample_label.grid(row=0, column=0)

    # Buttons
    start_button = Button(main_window, text="START", command=None)
    start_button.grid(row=0, column=1)

    capillary_maintenance_button = Button(main_window, text="Capillary Maintenance", command=capillary_maintenance_button_click)
    capillary_maintenance_button.grid(row=1, column=1)
