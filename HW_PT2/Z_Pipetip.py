...
class Z_Pipetip:
    ...
    def Move_ZP_start(self):
        print  "Moving Z Pipetip to right "
    ...
    def Move_ZP_Home(self):
        print  "Moving Z Pipetip to the home switch "
        target_motor = xyz_motor(5, 200, 100)
        atexit.register(target_motor.turn_off)
        HomeMax = 400
        Pos = 0
        print "Waiting for DAC addr5 input 2 Pipetip Home switch to become Active  high"
        temp = 0
        for cycle in range(1, 2):
            temp = temp + DAC.getDINbit(5, 2)
            time.sleep(.01)
        ...
        print "Completed Z Pipetip move to Home switch"
        if ((DAC.getDINbit(5, 2) == 1)):  # if home switch is active, reset LaserPos
            print "Input DAC(5,2) Z Pipetip Home sensor is Now Active high"
            print  Pos
            Pos = 0
