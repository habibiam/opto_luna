...
if __name__ == "__main__":
        ...
        #pfd = pifacedigitalio.PiFaceDigital(3)

        while True:
            print DAC.getADCall(0)
            time.sleep(1)
            temp = DAC.getADC(0,0)
            if ( temp >= 3.76 ):  # If higher than 3.727V, then cooler than 60 celcius
              #pfd.output_pins[0].value = 1       #Activate 24V to capillary heater pins NOpen and COMM
              print  "Heating ON, Capillary temperature voltage is "  
              print temp
              time.sleep(2)
            if ( temp <= 3.68 ):
              #pfd.output_pins[0].value = 0       #De-activate 24V to capillary heater pins NOpen and COMM
