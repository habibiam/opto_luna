		...
		if actual_move_steps >= 0:
			if step_mode == 1:
				self.stepper_motor.step(actual_move_steps, Opto_MotorHAT.FORWARD, Opto_MotorHAT.SINGLE, current)
                                time.sleep(.002)     #  Delay for the motor currents tomove the motor before next step
			elif step_mode == 2:
        ...
        while self._running:
           ...
           if Move_l_Laser_Enable:
	        ...
	        atexit.register(target_motor.turn_off)
	            ...
	            target_motor.move(NEGDIR, 1, FULLSTEP, HIGHCUR) # Retract one step at a time until home switch active
                    time.sleep(.001)
           ...
           if Move_r_Laser_Enable:
	        ...
	        atexit.register(target_motor.turn_off)
                for cycle in range (1,2000):
	            target_motor.move(POSDIR, 1, FULLSTEP, HIGHCUR) # Retract one step at a time until home switch active
                    time.sleep(.001)
	        ...
	        atexit.register(target_motor.turn_off)
                ...
                while ((temp == 0) and (GelPos < HomeMax) and (Move_GelPump_Home == 1)):   #if home switch not active, move 1 step
                      ...
                      for cycle in range (1,2):
                           temp = temp + DAC.getDINbit(7,0)
                           time.sleep(.01)
	              target_motor.move(POSDIR, 10, FASTSTEP, LOWCUR) # Go forward, up to top of gel pump to allow gel replacement
                      GelPos = GelPos + 1
           ...
           if Move_GelPump_Start :
	        ...
	        atexit.register(target_motor.turn_off)
                ...
                ave = 5.0
                while ((ave >= 0.8) and ( GelPos < HomeMax) and (Move_GelPump_Start == 1)):   #if gel start switch not active, move 9 step
                     temp = 0.0
                     ave = 0.0
                     good_count = 8
